...
.ocx-resize-bar {
  ...
  height: 4px;
  border-radius: 999px;
  background: var(--dsw-alias-label-tertiary, #cbd5e1);
...
.ocx-btn-icon.active {
  ...
  border-color: var(--dsw-alias-border-l1, #d1d5db);
}
...
`;
  ...
  function setupDraggable(el) {
    ...
    const handlePointerEnd = e => {
      ...
      if (hasMoved) {
        ...
        const SNAP_THRESHOLD = 90; // 靠近窗口边缘 90px 内自动吸附
        ...
        if (rect.top < SNAP_THRESHOLD) {
          finalTop = EDGE_GAP;
        } else if (screenH - (rect.top + pillH) < SNAP_THRESHOLD) {
          finalTop = screenH - pillH - EDGE_GAP;
        }

        // 防溢出安全保护
        finalLeft = Math.max(EDGE_GAP, Math.min(finalLeft, screenW - pillW - EDGE_GAP));
        finalTop = Math.max(EDGE_GAP, Math.min(finalTop, screenH - pillH - EDGE_GAP));
  ...
  function ensureMounted() {
    if (!pillElement || !document.body.contains(pillElement)) {
      ...
      pillElement.title = 'OpenCodex 额度监控（按住可拖拽移动，靠近边缘自动磁吸）';
  ...
  function render() {
    ...
    if (!isOnline) {
      contentHtml += `
        <div class="ocx-panel-scrollable" style="display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; color: var(--dsw-alias-label-secondary, #6b7280); padding: 18px 8px;">
      ...
      if (state.currentTab === 'providers') {
        if (state.isReordering) {
          contentHtml += `
            <div style="background: rgba(59, 130, 246, 0.08); border: 1px solid rgba(59, 130, 246, 0.2); border-radius: 8px; padding: 6px 10px; margin-bottom: 10px; font-size: 11px; color: var(--dsw-alias-state-business-primary, #3b82f6); display: flex; align-items: center; justify-content: space-between;">
