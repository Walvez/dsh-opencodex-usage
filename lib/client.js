...
`;
  ...
  function updatePillPosition() {
    ...
    if (Number.isFinite(state.pos.left)) {
      pillElement.style.left = `${state.pos.left}px`;
      pillElement.style.right = 'auto';
    } else {
      pillElement.style.right = `${state.pos.right || 24}px`;
      pillElement.style.left = 'auto';
    }
    pillElement.style.top = `${state.pos.top || 64}px`;
  }

  function updatePanelPosition() {
    if (!panelElement || !pillElement) return;
    const pillRect = pillElement.getBoundingClientRect();
    const panelWidth = 380;
    const panelHeight = Math.min(window.innerHeight * 0.8, 660);

    // X 轴定位
    if (pillRect.left + panelWidth > window.innerWidth - 16) {
      panelElement.style.right = `${Math.max(16, window.innerWidth - pillRect.right)}px`;
      panelElement.style.left = 'auto';
    } else {
      panelElement.style.left = `${Math.max(16, pillRect.left)}px`;
      panelElement.style.right = 'auto';
    }

    // Y 轴定位
    if (pillRect.bottom + panelHeight > window.innerHeight - 16 && pillRect.top > panelHeight) {
      panelElement.style.bottom = `${window.innerHeight - pillRect.top + 8}px`;
      panelElement.style.top = 'auto';
    } else {
      panelElement.style.top = `${pillRect.bottom + 8}px`;
  ...
  function renderProviderCard(report) {
    ...
    if (agg?.currentAccount) {
      ...
      const curQuota = agg.currentAccount.quota;
